...
class Avis {
    ...
    public static function getList(int $userId)
    {
        ...
        $stmt = $pdo->prepare("
            SELECT 
                a.*, u.nom AS auteur_nom, u.avatar AS auteur_avatar,
                t.depart AS ville_depart, t.arrivee AS ville_arrivee
            FROM avis a
            JOIN users u ON (
            ...
            JOIN trajets t ON t.id = a.trajet_id
            WHERE 
                (a.conducteur_id = :id AND a.auteur_role='passager')
             OR (a.passager_id   = :id AND a.auteur_role='conducteur')
            ORDER BY a.date_avis DESC
        ");
