...
class Reservation
{
    ...
    // Vérif si une réservation ACTIVE est déjà présente
    public static function existsForUser(int $userId, int $trajetId): bool
    {
        global $pdo;

        // On ne compte que les réservations 'en_attente' ou 'acceptee'
        // Cela permet de réserver à nouveau si la précédente a été refusée, expirée ou annulée (supprimée)
        $sql = $pdo->prepare("
            SELECT COUNT(*)
            FROM reservations
            WHERE passager_id = ? 
